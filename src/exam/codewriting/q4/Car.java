...
public class Car {
	...
	public Car(int fuelEfficiency, double fuelTank) {
	...
	*/
	public void drive ( double milesPerHour , double hours ) {
		double desiredMiles = milesPerHour*hours;
		double maxPossibleMiles = currentFuel*fuelEfficiency;
		double actualMiles = Math.min(desiredMiles, maxPossibleMiles);
		double gallons = actualMiles/fuelEfficiency;
	...
	*/
	public double addFuel ( double gallons ) {
		double extra = 0;    
    	if (currentFuel + gallons > fuelTank) {
        	extra = (currentFuel + gallons) - fuelTank;
